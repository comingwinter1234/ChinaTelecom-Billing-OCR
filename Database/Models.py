...
class Dept(Base):
    ...
    dept_id = Column(CHAR(60, 'utf8_unicode_ci'), primary_key=True)
    dept_name = Column(CHAR(60, 'utf8_unicode_ci'), nullable=False)


class Service(Base):
    __tablename__ = 'service'

    service_id = Column(CHAR(60, 'utf8_unicode_ci'), primary_key=True)
    service_name = Column(CHAR(60, 'utf8_unicode_ci'), nullable=False)


class Clas(Base):
    __tablename__ = 'class'

    class_id = Column(CHAR(60, 'utf8_unicode_ci'), primary_key=True)
    dept_id = Column(ForeignKey('dept.dept_id', ondelete='CASCADE', onupdate='CASCADE'), nullable=False, index=True)
    class_name = Column(CHAR(60, 'utf8_unicode_ci'), nullable=False)
...
class Worker(Base):
    ...
    worker_id = Column(CHAR(60, 'utf8_unicode_ci'), primary_key=True)
    class_id = Column(ForeignKey('class.class_id', ondelete='CASCADE', onupdate='CASCADE'), nullable=False, index=True)
    worker_name = Column(CHAR(60, 'utf8_unicode_ci'), nullable=False)
    role = Column(ENUM('normal', 'group', 'super'), nullable=False)
    pwd = Column(CHAR(120, 'utf8_unicode_ci'), nullable=False)
...
class ServiceRecord(Base):
    ...
    buyer_company = Column(CHAR(60, 'utf8_unicode_ci'))
    seller_company = Column(CHAR(60, 'utf8_unicode_ci'))
